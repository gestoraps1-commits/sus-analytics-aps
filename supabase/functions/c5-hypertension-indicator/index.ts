...
const buildFallbackPatient = (row: PatientInput, dueDate: Date): IndicatorPatient => {
  const flags = ([
    ...
    makeFlag(
      ...
      "Verifique nome, CPF, CNS e data de nascimento para permitir o cruzamento com os procedimentos do C5.",
      dueDate,
    ),
  );
...
Deno.serve(async (req) => {
  ...
  if (!rows.length) {
    return new Response(JSON.stringify({ success: false, error: "Nenhum paciente foi enviado para cálculo do indicador C5." }), {
      status: 400,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  ...
  try {
    ...
    const [attendanceResult, procedureResult, visitResult] = await Promise.all([
      client.query(
        ...
        `,
        [citizenIds],
      ).catch(e => { throw new Error(`Erro na tabela de atendimentos: ${e.message}`); }),
      client.query(
        `
          select
        ...
        `,
        [citizenIds],
      ).catch(e => { throw new Error(`Erro na tabela de procedimentos: ${e.message}`); }),
      client.query(
        `
          select
        ...
        `,
        [citizenIds],
      ).catch(e => { throw new Error(`Erro na tabela de visitas: ${e.message}`); }),
    ]);
    ...
    const matchedPatients: IndicatorPatient[] = patientsWithMatch.map(({ row, match }) => {
      ...
      const flagA = makeFlag(
        ...
        `Regra da nota metodológica C5: pelo menos 1 consulta presencial ou remota por médica(o) ou enfermeira(o) nos últimos 6 meses. Prazo operacional atual: ${formatDate(quadrimesterEnd.toISOString().slice(0, 10))}.`,
        quadrimesterEnd,
        consultationEvents,
      );
    ...
    console.error("c5-hypertension-indicator error", error);
    return new Response(
      JSON.stringify({
        success: false,
        error: error instanceof Error ? error.message : "Falha ao calcular o indicador C5.",
      }),
      {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
