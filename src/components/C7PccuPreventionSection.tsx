...
type C7PccuPreventionSectionProps = {
...
export const C7PccuPreventionSection = ({ 
  selectedSheet, 
  results, 
  referenceUploadId, 
  isPreparingData = false,
  initialSearch
}: C7PccuPreventionSectionProps) => {
  const { patients: indicatorPatients, isLoading, error } = useIndicatorData({
    config: INDICATOR_CONFIGS.c7,
    selectedSheet,
    results,
    referenceUploadId,
  });
  ...
  if (!selectedSheet) {
    if (isPreparingData) {
      return (
        <LoadingProgressCard
          title="Preparando dados do indicador C7"
          description="A aba C7 está sendo cruzada com a base para identificar PCCU, HPV, saúde sexual e reprodutiva e mamografia."
          value={38}
        />
      );
    }
    ...
    return (
      <Card className="overflow-hidden border-border/80 bg-card/90 shadow-sm">
        <CardContent className="px-6 py-12 text-center text-sm text-muted-foreground">
          Faça upload de uma planilha primeiro para gerar a análise do indicador C7.
        </CardContent>
      </Card>
    );
  }
  ...
  if (selectedSheet.mode !== "citizen") {
    return (
      <Card className="overflow-hidden border-border/80 bg-card/90 shadow-sm">
        <CardContent className="px-6 py-12 text-center text-sm text-muted-foreground">
          A seção C7 usa apenas abas de cidadãos com identificação individual para localizar procedimentos e vacinas na base.
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <Card className="overflow-hidden border-border/80 bg-card/90 shadow-sm">
        <CardContent className="space-y-5 p-5 md:p-6">
          <div className="grid gap-4 lg:grid-cols-4">
            ...
            <div className="rounded-3xl border border-border bg-background/70 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Pessoas monitoradas</p>
              <p className="mt-2 flex items-center gap-2 text-base font-semibold text-foreground">
          ...
          {isLoading ? (
            <LoadingProgressCard
              title="Calculando indicador C7"
              description="Buscando PCCU, vacina HPV, atendimentos de saúde sexual e reprodutiva e mamografia conforme a nota metodológica."
      ...
      {filteredPatients.length > 0 ? (
                      ...
                      <div className="space-y-4">
                        ...
                        {pendingFlags.length > 0 ? (
                          ...
                          <div className="rounded-3xl border border-primary/20 bg-primary/5 p-4 text-sm text-foreground">
                            Este registro não possui pendências vencidas no indicador C7 no momento.
                          </div>
                        )}
                      </div>
        ...
        <Card className="overflow-hidden border-border/80 bg-card/90 shadow-sm">
          <CardContent className="px-6 py-12 text-center text-sm text-muted-foreground">
            Nenhuma pessoa foi encontrada para calcular as flags do indicador C7.
          </CardContent>
        </Card>
      ) : null}
    </div>
