...
export const C4DiabetesSection = ({ 
  selectedSheet, 
  results, 
  referenceUploadId, 
  isPreparingData = false,
  initialSearch
}: C4DiabetesSectionProps) => {
  const { patients: indicatorPatients, isLoading, error } = useIndicatorData({
    config: INDICATOR_CONFIGS.c4,
    selectedSheet,
    results,
    referenceUploadId,
  });
    ...
    return indicatorPatients.filter((patient) => {
      ...
      const statusMatches =
        ...
        (filters.status === "tracking" && patient.trackingFlags > 0) ||
          ...
          (filters.status === "lost" && (patient.pendingFlags >= 3 || patient.totalPoints <= 25));
  ...
  if (!selectedSheet) {
    if (isPreparingData) {
      return (
        <LoadingProgressCard
          title="Preparando dados do indicador C4"
          description="A aba de pessoas com diabetes está sendo cruzada com a base para identificar consultas, procedimentos e pendências."
          value={38}
        />
      );
    }
    ...
    return (
      <Card className="overflow-hidden border-border/80 bg-card/90 shadow-sm">
        <CardContent className="px-6 py-12 text-center text-sm text-muted-foreground">
          Faça upload de uma planilha primeiro para gerar a análise de pessoas com diabetes.
        </CardContent>
      </Card>
    );
  }
  ...
  if (selectedSheet.mode !== "citizen") {
    return (
      <Card className="overflow-hidden border-border/80 bg-card/90 shadow-sm">
        <CardContent className="px-6 py-12 text-center text-sm text-muted-foreground">
          A seção C4 usa apenas abas de cidadãos com identificação individual para localizar pessoas com diabetes na base.
        </CardContent>
      </Card>
    );
  }
      ...
      <Card className="overflow-hidden border-border/80 bg-card/90 shadow-sm">
        <CardContent className="space-y-5 p-5 md:p-6">
          <div className="grid gap-4 lg:grid-cols-4">
            ...
            <div className="rounded-3xl border border-border bg-background/70 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Pessoas com diabetes</p>
              <p className="mt-2 flex items-center gap-2 text-base font-semibold text-foreground">
          ...
          {isLoading ? (
            <LoadingProgressCard
              title="Calculando indicador C4"
              description="Buscando consultas, pressão arterial, antropometria, visitas, hemoglobina glicada e avaliação dos pés na base remota."
      ...
      {filteredPatients.length > 0 ? (
        <div className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-border bg-background/60 px-4 py-3 text-sm text-muted-foreground">
            ...
            <span>Sem pendências: {dashboard.completed} · Críticos: {filteredPatients.filter((patient) => patient.pendingFlags >= 3 || patient.totalPoints <= 25).length}</span>
          </div>
                      ...
                      <div className="space-y-4">
                        ...
                        {pendingFlags.length > 0 ? (
                          ...
                          <div className="rounded-3xl border border-primary/20 bg-primary/5 p-4 text-sm text-foreground">
                            Este registro não possui pendências vencidas no indicador C4 no momento.
                          </div>
                        )}
                      </div>
        ...
        <Card className="overflow-hidden border-border/80 bg-card/90 shadow-sm">
          <CardContent className="px-6 py-12 text-center text-sm text-muted-foreground">
            Nenhuma pessoa com diabetes foi encontrada para calcular as flags do indicador C4.
          </CardContent>
        </Card>
      ) : null}
    </div>
