...
import { AlertCircle, CheckCircle2, ClipboardList, Database, FileSpreadsheet, Heart, Minus, Plus, ShieldAlert } from "lucide-react";
...
export const C3GestationSection = ({ 
  selectedSheet, 
  results, 
  referenceUploadId, 
  isPreparingData = false,
  initialSearch
}: C3GestationSectionProps) => {
  const { patients: indicatorPatients, isLoading, error } = useIndicatorData({
    config: INDICATOR_CONFIGS.c3,
    selectedSheet,
    results,
    referenceUploadId,
  });
  ...
  const procedureOptions = useMemo(
    () => Array.from(new Set(indicatorPatients.flatMap((patient) => patient.flags.map((flag) => `${flag.key} · ${flag.title}`)))).sort((a, b) => a.localeCompare(b, "pt-BR")),
    [indicatorPatients],
  );
  ...
  if (!selectedSheet) {
    if (isPreparingData) {
      return (
        <LoadingProgressCard
          title="Preparando dados do indicador C3"
          description="Os dados da planilha estão sendo cruzados para montar as regras de gestação e puerpério."
          value={38}
        />
      );
    }
    ...
    return (
      <Card className="overflow-hidden border-border/80 bg-card/90 shadow-sm">
        <CardContent className="px-6 py-12 text-center text-sm text-muted-foreground">
          Faça upload de uma planilha primeiro para gerar a análise de gestantes e puérperas.
        </CardContent>
      </Card>
    );
  }
  ...
  if (selectedSheet.mode !== "citizen") {
    return (
      <Card className="overflow-hidden border-border/80 bg-card/90 shadow-sm">
        <CardContent className="px-6 py-12 text-center text-sm text-muted-foreground">
          A seção C3 usa apenas abas de cidadãos com identificação individual para localizar as gestações e puerpérios.
        </CardContent>
      </Card>
    );
  }
      ...
      <Card className="overflow-hidden border-border/80 bg-card/90 shadow-sm">
        <CardContent className="space-y-5 p-5 md:p-6">
          <div className="grid gap-4 lg:grid-cols-4">
            ...
            <div className="rounded-3xl border border-border bg-background/70 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Gestantes/Puérperas</p>
              <p className="mt-2 flex items-center gap-2 text-base font-semibold text-foreground">
          ...
          {isLoading ? (
            <LoadingProgressCard
              title="Calculando indicador C3"
              description="Buscando pré-natal, puerpério, vacinação, exames, visitas e saúde bucal na base remota."
      ...
      {filteredPatients.length > 0 ? (
        <div className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-border bg-background/60 px-4 py-3 text-sm text-muted-foreground">
            <span>
              {hasActiveFilters
                ? `${filteredPatients.length} registro(s) encontrados após aplicar os filtros.`
                : `Exibindo ${visiblePatients.length} de ${filteredPatients.length} registro(s). Use os filtros para refinar.`}
            </span>
            <span>Sem pendências: {dashboard.completed} · Críticos: {filteredPatients.filter((patient) => patient.pendingFlags >= 4 || patient.totalPoints <= 25).length}</span>
          </div>
          ...
          <div className="space-y-5">
            {visiblePatients.map((patient) => {
              ...
              const recentAnthropometryRecords = (patient.anthropometryRecords ?? [])
                .filter((record) => record?.date && record.weight !== null && record.weight !== undefined && record.height !== null && record.height !== undefined)
                      ...
                      <div className="space-y-4">
                        <div className="flex flex-wrap items-start justify-between gap-4">
                          <div className="space-y-2">
                            ...
                            <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
                              ...
                              <span>Referência: {formatDate(patient.nascimento)}</span>
                          ...
                          <div className="flex items-start gap-3">
                            <div className="min-w-[220px] rounded-3xl border border-border bg-background/70 p-4">
                              ...
                              {isExpanded ? <AnthropometrySummary records={recentAnthropometryRecords} /> : null}
                            </div>
                        ...
                        {pendingFlags.length > 0 ? (
                          ...
                          <div className="rounded-3xl border border-primary/20 bg-primary/5 p-4 text-sm text-foreground">
                            Este registro não possui pendências vencidas no indicador C3 no momento.
                          </div>
                        )}
                      </div>
                                  ...
                                  {flag.events?.length && !isAnthropometryFlag(flag) ? (
                                    <div className="mt-3 rounded-2xl border border-border/70 bg-background/80 p-3">
                                      ...
                                      <ul className="mt-2 max-h-40 space-y-1 overflow-y-auto text-xs leading-5 text-foreground">
                                        {flag.events.map((event, eventIndex) => (
          ...
          {!hasActiveFilters && displayLimit < filteredPatients.length ? (
            <div className="flex justify-center pt-2">
              <Button type="button" variant="outline" className="rounded-full px-6" onClick={() => setDisplayLimit((prev) => prev + 50)}>
                Carregar mais {Math.min(50, filteredPatients.length - displayLimit)} registros
              </Button>
            </div>
          ) : null}
        </div>
        ...
        <Card className="overflow-hidden border-border/80 bg-card/90 shadow-sm">
          <CardContent className="px-6 py-12 text-center text-sm text-muted-foreground">
            Nenhuma gestante ou puérpera foi encontrada para calcular as flags do indicador C3.
          </CardContent>
        </Card>
      ) : null}
    </div>
